...
func flip_to_right():
	var h_bg = $HealthBarBackground
	var e_bg = $EnergyBarBackground
	
	if h_bg and e_bg:
		# En lugar de usar viewport (que puede fallar si la ventana cambia), 
		# usamos anchors para que se alinee automáticamente a la derecha
		h_bg.anchor_left = 1.0
		h_bg.anchor_right = 1.0
		h_bg.offset_left = -340.0 - 10.0
		h_bg.offset_right = -10.0
		
		e_bg.anchor_left = 1.0
		e_bg.anchor_right = 1.0
		e_bg.offset_left = -340.0 - 10.0
		e_bg.offset_right = -10.0
		
		# Ocultar Wave/Timer/Enemies en la UI del Jugador 2 para que no se dupliquen visualmente
		if wave_label: wave_label.hide()
		if timer_label: timer_label.hide()
		if enemies_left_label: enemies_left_label.hide()
